...
public class DiscussBox extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String discussTopicId="",user_id="",discussCategory="",discuss_content="",relpath="0";
		try
		{
			discuss_topic_id = request.getSession(false);
			discussTopicId = (String)discuss_topic_id.getAttribute("discuss_topic_id");
			...
			discuss_content = request.getParameter("discuss_content");
			if(discuss_content==null)
			{
				discuss_content="";
			}
			...
			String sql = "select count(id) from "+discussCategory+" where topic=?";
			...
			sql = "insert into "+discussCategory+"(id,type,name,posted_by,topic,picture,uid) values(?,?,?,?,?,?,?)";
			ps = cn.prepareStatement(sql);
			ps.setString(1, "Discuss_"+count);
			ps.setString(2, "Discuss");
			ps.setString(3, discuss_content);
			ps.setString(4, user_id);
			ps.setString(5, discussTopicId);
			ps.setString(6, relpath);
			ps.setString(7, String.valueOf(count2));
			ps.execute();
